...
export default function HomePage() {
  // Declara o componente principal da página.
  ...
  const handleSubmit = async (e) => {
    // Gerencia o envio do formulário (inserção ou atualização de livros).
    e.preventDefault(); // Evita o comportamento padrão de recarregar a página.
    const newBook = { title, author }; // Cria um objeto com os dados do livro.

    try {
      if (editItem) {
        // Se há um item em edição:
  ...
  const handleDelete = async (id) => {
    // Gerencia a exclusão de um livro.
    try {
  ...
  const handleEdit = (book) => {
    // Prepara um item para ser editado.
  ...
  useEffect(() => {
    // Executa um efeito colateral ao carregar o componente.
  ...
  return (
    <>
      ...
      <section className="books-page">
        ...
        <h1>{editItem ? "Editar Item" : "Inserir Item"}</h1>{" "}
        {/* Mostra o título dinâmico com base na ação. */}
        <form className="new-book" onSubmit={handleSubmit}>
          {" "}
          {/* Formulário para adicionar ou editar livros. */}
          <div>
            <label htmlFor="titulo">Título: </label>{" "}
            {/* Rótulo para o campo de título. */}
            <input
              type="text"
              id="titulo"
              name="titulo"
          ...
          <button type="submit">{editItem ? "Atualizar" : "Inserir"}</button>{" "}
          {/* Botão dinâmico com base na ação. */}
        </form>
        <h1>Tabela de Livros</h1> {/* Título da tabela */}
        <table>
          {" "}
          {/* Início da tabela de livros */}
          <thead>
            {" "}
            {/* Cabeçalho da tabela */}
            <tr>
          ...
          <tbody>
            ...
            {books.map((book) => {
              ...
              return (
                <tr key={book.ID}>
                  {" "}
                  {/* Linha da tabela para cada livro. */}
